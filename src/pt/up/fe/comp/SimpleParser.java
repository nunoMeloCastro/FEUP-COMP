...
public class SimpleParser implements JmmParser {
    ...
    public JmmParserResult parse(String jmmCode, String startingRule, Map<String, String> config) {
        ...
        try {
            ...
            ParseException p = TestUtils.getException(e, ParseException.class);
            if(p != null)
            {
                int line = p.getToken().getBeginLine();
                int column = p.getToken().getBeginColumn();
                String message = p.getMessage();
                return JmmParserResult.newError(Report.newError(Stage.SYNTATIC, line, column, message, p));
            }

            return JmmParserResult.newError(Report.newError(Stage.SYNTATIC, -1, -1, "error while parsing", e));
